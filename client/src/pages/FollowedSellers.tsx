...
import { listMyPortableFollows, portableMediaUrl } from "@/lib/marketplaceSupabase";
...
export default function FollowedSellers() {
  ...
  const legacyFollowed = trpc.marketplace.follows.mine.useQuery(undefined, { enabled: !isSupabaseMode && isAuthenticated, refetchOnWindowFocus: true });
  const portableFollowed = useQuery({ queryKey: ["portable-followed-sellers"], queryFn: listMyPortableFollows, enabled: isSupabaseMode && isAuthenticated, refetchOnWindowFocus: true });
  const followed = isSupabaseMode ? portableFollowed : legacyFollowed;
