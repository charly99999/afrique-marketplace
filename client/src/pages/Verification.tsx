...
export default function Verification() {
  ...
  const [selfieLiveness, setSelfieLiveness] = useState<"passed" | "not_checked">("not_checked");
  const [mediaIssue, setMediaIssue] = useState<string>();
  const [aiMessage, setAiMessage] = useState<string>();
  const [aiStarted, setAiStarted] = useState(false);
  const [localPreflightPending, setLocalPreflightPending] = useState(false);
  ...
  const verificationPending = submitPending || localPreflightPending;
  const analyzePending = isSupabaseMode ? false : legacyAnalyzePending.isPending;
  const portableVerification = isSupabaseMode ? existing.data as PortableVerification | null | undefined : null;
  const portableAwaitingConfirmedAnalysis = portableVerification?.status === "pending" && portableVerification.retryAllowed;
  const portableInManualReview = portableVerification?.status === "pending" && !portableVerification.retryAllowed;
  const portablePendingVerificationId = portableVerification?.status === "pending" ? portableVerification.id : null;
