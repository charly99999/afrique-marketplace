...
export default function Sell() {
  ...
  if (profile.data.verificationStatus !== "verified") return <MarketplaceShell title="Publier"><section className="page-wrap section-space"><div className="gate-card"><LockKeyhole size={28} /><h2>{profile.data.verificationStatus === "pending" ? "Votre dossier est en cours d’examen." : "La publication attend votre vérification."}</h2><p>{profile.data.verificationStatus === "pending" ? "Votre document et votre selfie sont enregistrés. La publication sera disponible dès qu’une décision de vérification sera prise." : "Votre profil doit être vérifié afin de protéger les acheteurs et vendeurs."}</p><Link href="/verification" className="button button--gold">Voir ma vérification</Link></div></section></MarketplaceShell>;
